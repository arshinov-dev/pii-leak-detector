...
```bash
python main.py share --extract --extract-limit 50
```

Можно совместить со сводкой планов:

```bash
python main.py share --plan --extract --extract-limit 50
```
...
- OCR изображений и PDF-страниц.
- Старые `.doc` и `.xls` без внешних конвертеров.
- Полноценная обработка Parquet без `pyarrow`.
- PPT/PPTX.
- Безопасная распаковка архивов.

Эти форматы остаются в планах как будущие шаги, но базовый слой уже дает
нормальный текст для PDF с цифровым слоем, HTML, DOCX, CSV, JSON, TXT и RTF.
